...
def similarity(a, b):
    ...
    for i in xrange(0,len(a)):
        result = result + ((a[i] - meanA)*(b[i] - meanB))
...
def computeDelta(wt, X, Xi):
    ...
    #get row of X without yi
    X1 = (X.iloc[0:(len(X)-1)])
    #sigma(wt*exponent(similarity))
    similaritySum=0.0
    #sigma(yi*similarity)
    expectedValSum=0.0
    #repeat for all values of Xi except last
    for i in range(0, len(Xi)):
        #take row of Xi
        XiR = Xi.iloc[i]
        #calculate similarity between X1 and 
        similarity0=np.exp(wt*similarity(X1, XiR.iloc[0:(len(XiR)-1)]))
        #add similarity to similaritySum
        similaritySum=similaritySum+similarity0
        #calculated expectedVal from yi
        expectedVal=similarity0*XiR[-1]
        #add expectedVal to expectedValSum
        expectedValSum=expectedValSum+expectedVal
    
    return expectedValSum/similaritySum
